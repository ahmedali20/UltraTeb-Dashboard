...
export default function InvoiceDetailsClient({ invoice, notes, customer, wht, collections, chequeAllocations, cogs, isAdmin, canViewCollections }: { invoice: Invoice; notes: Note[]; customer: { customer_official_name: string | null; payment_terms_days: number | null } | null; wht: Wht[]; collections: Collection[]; chequeAllocations: ChequeAllocation[]; cogs: Cogs[]; isAdmin: boolean; canViewCollections: boolean }) {
  ...
  const collectedTotal = customerCollectionsTotal + deductedWhtTotal;
    ...
    <div className="dashboard-shell invoice-details-shell" dir={lang === "ar" ? "rtl" : "ltr"}>
      ...
      <main className="invoice-details-page">
        ...
        <div className="invoice-details-grid invoice-details-grid--top">
          ...
          <section className="invoice-detail-card"><h2>Settlement Signals</h2><p className="invoice-detail-note">{canViewCollections ? "Settled includes cleared customer payments plus WHT actually deducted by the customer." : "Collection details require Collections view permission."}</p><div className="invoice-signal-grid"><article><span>Adjusted Invoice</span><strong>{money(adjustedSales)}</strong></article>{canViewCollections && <><article><span>Customer Payments</span><strong>{money(customerCollectionsTotal)}</strong></article><article><span>WHT Deducted</span><strong>{money(deductedWhtTotal)}</strong></article><article><span>Total Settled</span><strong>{money(collectedTotal)}</strong></article><article><span>{remaining < 0 ? "Overpaid" : "Remaining"}</span><strong>{money(Math.abs(remaining))}</strong></article></>}</div></section>
        </div>
        ...
        {canViewCollections && <section className="invoice-detail-card">
          ...
          {collections.length > 0 && <div className="invoice-collection-table"><table><thead><tr><th>Collection Date</th><th>Method</th><th>Status</th><th>Reference</th><th>Transfer Fees</th><th>Cash Fraction</th><th>WHT Deducted</th><th>Notes</th><th>Collected Amount</th></tr></thead><tbody>{collections.map((collection) => <tr key={collection.id}><td>{dateLabel(collection.collection_date)}</td><td>{collection.payment_method.replaceAll("_", " ")}</td><td>Completed</td><td>{collection.reference_no || "—"}</td><td>{Number(collection.transfer_fees || 0) ? money(collection.transfer_fees) : "—"}</td><td>{Number(collection.cash_fraction || 0) ? money(collection.cash_fraction) : "—"}</td><td>{Number(collection.wht_deducted_amount || 0) ? money(collection.wht_deducted_amount) : "No"}</td><td>{collection.notes || "—"}</td><td><strong>{money(collection.amount)}</strong></td></tr>)}</tbody></table></div>}
          {chequeAllocations.length > 0 && <div className="invoice-cheque-history"><h3>Linked Cheques</h3><div className="invoice-collection-table"><table><thead><tr><th>Cheque Date</th><th>Cheque No.</th><th>Status</th><th>Notes</th><th>Cheque Total</th><th>Allocated to Invoice</th><th>WHT Deducted</th></tr></thead><tbody>{chequeAllocations.map((allocation) => <tr key={allocation.id}><td>{dateLabel(allocation.cheque?.cheque_date || null)}</td><td>{allocation.cheque?.cheque_no || "—"}</td><td><span className={`cheque-badge cheque-badge--${(allocation.cheque?.cheque_status || "IN_TREASURY").toLowerCase()}`}>{(allocation.cheque?.cheque_status || "IN_TREASURY").replaceAll("_", " ")}</span></td><td>{allocation.cheque?.notes || "—"}</td><td>{money(allocation.cheque?.amount || 0)}</td><td><strong>{money(allocation.allocated_amount)}</strong></td><td>{Number(allocation.wht_deducted_amount || 0) ? money(allocation.wht_deducted_amount) : "No"}</td></tr>)}</tbody></table></div></div>}
