...
type Collection = { id: number; collection_date: string; amount: number; payment_method: string; reference_no: string | null; notes: string | null };
...
export default function InvoiceDetailsClient({ invoice, notes, customer, wht, collections, cogs, isAdmin, canViewCollections }: { invoice: Invoice; notes: Note[]; customer: { customer_official_name: string | null; payment_terms_days: number | null } | null; wht: Wht[]; collections: Collection[]; cogs: Cogs[]; isAdmin: boolean; canViewCollections: boolean }) {
  ...
  const collectedTotal = collections.reduce((sum, item) => sum + Number(item.amount || 0), 0);
    ...
    <div className="dashboard-shell invoice-details-shell" dir={lang === "ar" ? "rtl" : "ltr"}>
      ...
      <main className="invoice-details-page">
        ...
        <div className="invoice-details-grid invoice-details-grid--top">
          ...
          <section className="invoice-detail-card"><h2>Settlement Signals</h2><p className="invoice-detail-note">{canViewCollections ? "The balance includes linked credit and debit notes and every recorded customer collection." : "Collection details require Collections view permission."}</p><div className="invoice-signal-grid"><article><span>Adjusted Invoice</span><strong>{money(adjustedSales)}</strong></article>{canViewCollections && <><article><span>Collected</span><strong>{money(collectedTotal)}</strong></article><article><span>{remaining < 0 ? "Overpaid" : "Remaining"}</span><strong>{money(Math.abs(remaining))}</strong></article></>}</div></section>
        </div>
        ...
        {canViewCollections && <section className="invoice-detail-card"><div className="invoice-collection-heading"><h2>Collection History</h2><a href={`/collections?invoice=${encodeURIComponent(String(invoice.id))}`}>Manage Collections</a></div>{collections.length ? <div className="invoice-collection-table"><table><thead><tr><th>Collection Date</th><th>Method</th><th>Reference</th><th>Notes</th><th>Amount</th></tr></thead><tbody>{collections.map((collection) => <tr key={collection.id}><td>{dateLabel(collection.collection_date)}</td><td>{collection.payment_method.replaceAll("_", " ")}</td><td>{collection.reference_no || "—"}</td><td>{collection.notes || "—"}</td><td><strong>{money(collection.amount)}</strong></td></tr>)}</tbody><tfoot><tr><td colSpan={4}>Total Collected</td><td><strong>{money(collectedTotal)}</strong></td></tr></tfoot></table></div> : <p className="invoice-detail-empty">No customer collection has been recorded for this invoice.</p>}</section>}
