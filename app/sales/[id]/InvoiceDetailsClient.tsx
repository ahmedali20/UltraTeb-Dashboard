    ...
    <div className="dashboard-shell invoice-details-shell" dir={lang === "ar" ? "rtl" : "ltr"}>
      ...
      <main className="invoice-details-page">
        ...
        {canViewCollections && <section className="invoice-detail-card">
          ...
          {collections.length > 0 && <div className="invoice-collection-table"><table><thead><tr><th>Collection Date</th><th>Method</th><th>Status</th><th>Reference</th><th>Transfer Fees</th><th>Cash Fraction</th><th>WHT Deducted</th><th>Notes</th><th>Invoice Payment</th></tr></thead><tbody>{collections.map((collection) => <tr key={collection.id}><td>{dateLabel(collection.collection_date)}</td><td>{collection.payment_method.replaceAll("_", " ")}</td><td>Completed</td><td>{collection.reference_no || "—"}</td><td>{Number(collection.transfer_fees || 0) ? money(collection.transfer_fees) : "—"}</td><td>{Number(collection.cash_fraction || 0) ? money(collection.cash_fraction) : "—"}</td><td>{Number(collection.wht_deducted_amount || 0) ? money(collection.wht_deducted_amount) : "No"}</td><td>{collection.notes || "—"}</td><td><strong>{money(Math.max(0, Number(collection.amount || 0) - Number(collection.transfer_fees || 0)))}</strong></td></tr>)}</tbody></table></div>}
