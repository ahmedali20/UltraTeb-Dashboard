...
type Collection = { id: number; collection_date: string; amount: number; transfer_fees: number; payment_method: string; cheque_status: string | null; cheque_status_date: string | null; reference_no: string | null; notes: string | null };
    ...
    <div className="dashboard-shell invoice-details-shell" dir={lang === "ar" ? "rtl" : "ltr"}>
      ...
      <main className="invoice-details-page">
        ...
        {canViewCollections && <section className="invoice-detail-card">
          ...
          {collections.length > 0 && <div className="invoice-collection-table"><table><thead><tr><th>Collection Date</th><th>Method</th><th>Status</th><th>Reference</th><th>Transfer Fees</th><th>Notes</th><th>Collected Amount</th></tr></thead><tbody>{collections.map((collection) => <tr key={collection.id}><td>{dateLabel(collection.collection_date)}</td><td>{collection.payment_method.replaceAll("_", " ")}</td><td>Completed</td><td>{collection.reference_no || "—"}</td><td>{Number(collection.transfer_fees || 0) ? money(collection.transfer_fees) : "—"}</td><td>{collection.notes || "—"}</td><td><strong>{money(collection.amount)}</strong></td></tr>)}</tbody></table></div>}
