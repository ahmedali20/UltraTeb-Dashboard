...
export default function InvoiceDetailsClient({ invoice, notes, customer, wht, collections, cogs, isAdmin, canViewCollections }: { invoice: Invoice; notes: Note[]; customer: { customer_official_name: string | null; payment_terms_days: number | null } | null; wht: Wht[]; collections: Collection[]; cogs: Cogs[]; isAdmin: boolean; canViewCollections: boolean }) {
  ...
  const customerCollectionsTotal = collections.reduce((sum, item) => sum + (item.payment_method !== "CHEQUE" || item.cheque_status === "COLLECTED" ? Number(item.amount || 0) : 0), 0);
  ...
  return (
    <div className="dashboard-shell invoice-details-shell" dir={lang === "ar" ? "rtl" : "ltr"}>
      ...
      <main className="invoice-details-page">
        ...
        {canViewCollections && <section className="invoice-detail-card"><div className="invoice-collection-heading"><h2>Collection History</h2><a href={`/collections?invoice=${encodeURIComponent(String(invoice.id))}`}>Manage Collections</a></div>{collections.length ? <div className="invoice-collection-table"><table><thead><tr><th>Collection Date</th><th>Method</th><th>Status</th><th>Reference</th><th>Notes</th><th>Amount</th></tr></thead><tbody>{collections.map((collection) => <tr key={collection.id}><td>{dateLabel(collection.collection_date)}</td><td>{collection.payment_method.replaceAll("_", " ")}</td><td>{collection.cheque_status ? collection.cheque_status.replaceAll("_", " ") : "Completed"}</td><td>{collection.reference_no || "—"}</td><td>{collection.notes || "—"}</td><td><strong>{money(collection.amount)}</strong></td></tr>)}</tbody><tfoot><tr><td colSpan={5}>Cleared Customer Payments</td><td><strong>{money(customerCollectionsTotal)}</strong></td></tr></tfoot></table></div> : <p className="invoice-detail-empty">No customer collection has been recorded for this invoice.</p>}</section>}
