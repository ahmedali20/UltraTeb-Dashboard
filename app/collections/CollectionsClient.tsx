...
const emptyForm = { invoiceId: "", collectionDate: new Date().toISOString().slice(0, 10), amount: "", paymentMethod: "BANK_TRANSFER", referenceNo: "", notes: "" };
...
export default function CollectionsClient({ invoices, initialCollections, canEdit }: { invoices: Invoice[]; initialCollections: Collection[]; canEdit: boolean }) {
  ...
  const collectedByInvoice = useMemo(() => records.reduce((map, record) => map.set(record.invoice_id, (map.get(record.invoice_id) ?? 0) + Number(record.amount || 0)), new Map<string, number>()), [records]);
  ...
  const totalCollected = filtered.reduce((sum, record) => sum + Number(record.amount || 0), 0);
  ...
  useEffect(() => {
    ...
    if (invoiceId && invoiceMap.has(invoiceId)) setForm((current) => ({ ...current, invoiceId }));
  }, [invoiceMap]);
  ...
  function edit(record: Collection) { setEditingId(record.id); setForm({ invoiceId: record.invoice_id, collectionDate: record.collection_date, amount: String(record.amount), paymentMethod: record.payment_method, referenceNo: record.reference_no ?? "", notes: record.notes ?? "" }); window.scrollTo({ top: 0, behavior: "smooth" }); }
  ...
  return <div className="dashboard-shell" dir={lang === "ar" ? "rtl" : "ltr"}>
    ...
    <main className="collections-page">
      <section className="collections-hero"><div><p>ACCOUNTS RECEIVABLE</p><h1>Invoice Collections</h1><span>Record partial or full customer payments and track outstanding balances.</span></div><article><span>Filtered Collection</span><strong>EGP {money(totalCollected)}</strong></article></section>
      {canEdit && <section className="collection-form-card"><div className="collections-heading"><div><p>{editingId ? "EDIT PAYMENT" : "NEW PAYMENT"}</p><h2>{editingId ? "Update collection" : "Record a collection"}</h2></div>{editingId && <button onClick={reset}>Cancel Edit</button>}</div><div className="collection-form-grid">
        <label className="collection-invoice-field"><span>Invoice *</span><select value={form.invoiceId} onChange={(event) => update("invoiceId", event.target.value)} disabled={Boolean(editingId)}><option value="">Select customer and invoice</option>{invoices.map((invoice) => <option key={String(invoice.id)} value={String(invoice.id)}>{invoice.customer_name} — Invoice {invoice.invoice_no} — EGP {money(invoice.total_sales)}</option>)}</select></label>
        ...
        <label><span>Cheque / Reference No.</span><input value={form.referenceNo} onChange={(event) => update("referenceNo", event.target.value)} /></label>
