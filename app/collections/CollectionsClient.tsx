...
export default function CollectionsClient({ invoices, initialCollections, initialChequeAllocations, initialWht, canEdit }: { invoices: Invoice[]; initialCollections: Collection[]; initialChequeAllocations: ChequeAllocation[]; initialWht: WhtCollection[]; canEdit: boolean }) {
  ...
  function edit(record: Collection) { const fees = Number(record.transfer_fees || 0); const fraction = Number(record.cash_fraction || 0); setEditingId(record.id); setApplyWht(Number(record.wht_deducted_amount || 0) > 0); setForm({ customerName: record.customer_name, invoiceId: record.invoice_id, collectionDate: record.collection_date, chequeDate: record.collection_date, bankName: "", amount: String(Math.max(0, Number(record.amount) - fees)), transferFees: fees ? String(fees) : "", cashFraction: fraction ? String(fraction) : "", paymentMethod: record.payment_method, referenceNo: record.reference_no ?? "", notes: record.notes ?? "" }); window.scrollTo({ top: 0, behavior: "smooth" }); }

  async function save() {
    setSaving(true);
    try {
      const allocatedInvoices = Object.entries(allocations).filter(([, value]) => Number(value) > 0).map(([invoiceId, amount]) => ({ invoiceId, amount: Number(amount), whtDeductedAmount: whtDeductions[invoiceId] && (deductedWhtByInvoice.get(invoiceId) ?? 0) <= 0.005 ? (whtByInvoice.get(String(invoiceMap.get(invoiceId)?.invoice_no)) ?? 0) : 0 }));
      ...
      setRecords((current) => editingId ? current.map((record) => record.id === editingId ? savedRecords[0] : record) : [...savedRecords, ...current]);
      reset();
  ...
  const multiTransfer = form.paymentMethod === "BANK_TRANSFER" && !editingId;
