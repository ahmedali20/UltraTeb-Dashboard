...
export default function CollectionsClient({ invoices, initialCollections, canEdit }: { invoices: Invoice[]; initialCollections: Collection[]; canEdit: boolean }) {
  ...
  const collectedByInvoice = useMemo(() => records.reduce((map, record) => map.set(record.invoice_id, (map.get(record.invoice_id) ?? 0) + clearedAmount(record)), new Map<string, number>()), [records]);
  ...
  const totalCollected = filtered.reduce((sum, record) => sum + clearedAmount(record), 0);
