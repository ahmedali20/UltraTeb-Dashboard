...
type ChequeAllocation = { id: number; cheque_id: number; invoice_id: string; invoice_no: string; allocated_amount: number; cheque: { id: number; cheque_no: string; cheque_date: string; cheque_status: string; cheque_status_date: string; customer_name: string; amount: number; notes: string | null } | null };
...
export default function CollectionsClient({ invoices, initialCollections, initialChequeAllocations, initialWht, canEdit }: { invoices: Invoice[]; initialCollections: Collection[]; initialChequeAllocations: ChequeAllocation[]; initialWht: WhtCollection[]; canEdit: boolean }) {
  ...
  const chequeValid = form.paymentMethod === "CHEQUE" && Boolean(form.customerName && form.referenceNo && Number(form.amount) > 0 && allocatedTotal > 0 && Math.abs(allocatedTotal - Number(form.amount)) <= .01);
    ...
    <main className="collections-page">
      ...
      {canEdit && <section className="collection-form-card">
        ...
        <div className="collection-form-grid">
          ...
          <label><span>Collection / Cheque Date *</span><input type="date" value={form.collectionDate} onChange={(event) => update("collectionDate", event.target.value)} /></label>
        ...
        {form.paymentMethod === "CHEQUE" && form.customerName && <div className="cheque-allocation-panel">
          ...
          <div className="cheque-allocation-list">{customerInvoices.map((invoice) => { const remaining = Number(invoice.total_sales) - (settledByInvoice.get(String(invoice.id)) ?? 0); return <label key={String(invoice.id)}><span><strong>Invoice {invoice.invoice_no}</strong><small>{dateLabel(invoice.sales_date)} · Remaining EGP {money(remaining)}</small></span><input type="number" min="0" max={Math.max(0, remaining)} step="0.01" placeholder="Allocation" value={allocations[String(invoice.id)] ?? ""} onChange={(event) => setAllocations((current) => ({ ...current, [String(invoice.id)]: event.target.value }))} /></label>; })}</div>
        </div>}
