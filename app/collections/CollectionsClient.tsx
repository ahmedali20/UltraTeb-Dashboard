...
type Collection = { id: number; invoice_id: string; invoice_no: string; customer_name: string; collection_date: string; amount: number; transfer_fees: number; payment_method: string; reference_no: string | null; notes: string | null };
...
const emptyForm = { customerName: "", invoiceId: "", collectionDate: today, chequeDate: today, bankName: "", amount: "", transferFees: "", paymentMethod: "CHEQUE", referenceNo: "", notes: "" };
...
export default function CollectionsClient({ invoices, initialCollections, initialChequeAllocations, initialWht, canEdit }: { invoices: Invoice[]; initialCollections: Collection[]; initialChequeAllocations: ChequeAllocation[]; initialWht: WhtCollection[]; canEdit: boolean }) {
  ...
  function edit(record: Collection) { const fees = Number(record.transfer_fees || 0); setEditingId(record.id); setForm({ customerName: record.customer_name, invoiceId: record.invoice_id, collectionDate: record.collection_date, chequeDate: record.collection_date, bankName: "", amount: String(Math.max(0, Number(record.amount) - fees)), transferFees: fees ? String(fees) : "", paymentMethod: record.payment_method, referenceNo: record.reference_no ?? "", notes: record.notes ?? "" }); window.scrollTo({ top: 0, behavior: "smooth" }); }
  ...
  const collectedTotal = (Number(form.amount) || 0) + transferFees;
    ...
    <main className="collections-page">
      ...
      {canEdit && <section className="collection-form-card">
        ...
        <div className="collection-form-grid">
          ...
          <label><span>Payment Method *</span><select value={form.paymentMethod} disabled={Boolean(editingId)} onChange={(event) => { setForm((current) => ({ ...current, paymentMethod: event.target.value, transferFees: "" })); setAllocations({}); }}><option value="BANK_TRANSFER">Bank Transfer</option><option value="CHEQUE">Cheque</option><option value="CASH">Cash</option><option value="OTHER">Other</option></select></label>
      ...
      <section className="collection-table-card"><div className="collections-toolbar"><div><h2>Non-Cheque Collection History</h2><span>{filtered.length} records</span></div><select value={customerFilter} onChange={(event) => setCustomerFilter(event.target.value)}><option value="All">All Customers</option>{customers.map((customer) => <option key={customer}>{customer}</option>)}</select><input placeholder="Search invoice, customer or reference" value={search} onChange={(event) => setSearch(event.target.value)} /></div><div className="table-scroll"><table><thead><tr><th>Date</th><th>Customer</th><th>Invoice</th><th>Method</th><th>Reference</th><th>Bank Amount</th><th>Transfer Fees</th><th>Collected Amount</th><th>Invoice Balance</th><th>Notes</th>{canEdit && <th>Actions</th>}</tr></thead><tbody>{filtered.map((record) => { const invoice = invoiceMap.get(record.invoice_id); const balance = Number(invoice?.total_sales ?? 0) - (settledByInvoice.get(record.invoice_id) ?? 0); const fees = Number(record.transfer_fees || 0); return <tr key={record.id}><td>{dateLabel(record.collection_date)}</td><td>{record.customer_name}</td><td><a href={`/sales/${record.invoice_id}`}>{record.invoice_no}</a></td><td>{record.payment_method.replaceAll("_", " ")}</td><td>{record.reference_no || "—"}</td><td>EGP {money(Number(record.amount) - fees)}</td><td>{fees ? `EGP ${money(fees)}` : "—"}</td><td><strong>EGP {money(record.amount)}</strong></td><td className={balance <= 0 ? "collection-paid" : ""}>EGP {money(balance)}</td><td>{record.notes || "—"}</td>{canEdit && <td><div className="collection-row-actions"><button onClick={() => edit(record)}>Edit</button><button className="danger" onClick={() => remove(record.id)}>Delete</button></div></td>}</tr>; })}{!filtered.length && <tr><td colSpan={canEdit ? 11 : 10} className="collection-empty">No collection records found.</td></tr>}</tbody></table></div></section>
