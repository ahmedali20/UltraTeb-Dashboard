...
type VatRecord = { id: string; invoice_no: string; sales_date: string; document_type: "INVOICE" | "CR_NOTE" | "DR_NOTE"; vat_customer_name: string; customer_trn: string; customer_address: string; sales_item_total: number; tax: number; vat_amount: number; table_tax_amount: number; tax_classification: string; total_sales: number; wht: number };
...
export default function VatSalesClient({ records }: { records: VatRecord[] }) {
  ...
  const totals = useMemo(() => filtered.reduce((sum, record) => {
    ...
    return { net: sum.net + sign * Math.abs(Number(record.sales_item_total || 0)), vat: sum.vat + sign * Math.abs(Number(record.vat_amount || 0)), tableTax: sum.tableTax + sign * Math.abs(Number(record.table_tax_amount || 0)), total: sum.total + sign * Math.abs(Number(record.total_sales || 0)), wht: sum.wht + Number(record.wht || 0) };
  }, { net: 0, vat: 0, tableTax: 0, total: 0, wht: 0 }), [filtered]);
  ...
  return <div className="dashboard-shell vat-report-shell" dir={lang === "ar" ? "rtl" : "ltr"}>
    ...
    <main className="vat-report-page">
      ...
      <section className="vat-report-kpis"><article><span>Records</span><strong>{filtered.length}</strong></article><article><span>Net Amount</span><strong>{money(totals.net)}</strong></article><article><span>Net VAT</span><strong>{money(totals.vat)}</strong></article><article><span>Table Tax</span><strong>{money(totals.tableTax)}</strong></article><article><span>Grand Total</span><strong>{money(totals.total)}</strong></article></section>
      ...
      <section className="vat-report-table"><div className="table-scroll"><table><thead><tr><th>Document</th><th>Invoice No.</th><th>Customer</th><th>Tax Registration No.</th><th>Address</th><th>Date</th><th>Net Amount</th><th>VAT</th><th>Table Tax</th><th>Tax Type</th><th>WHT</th><th>Total</th></tr></thead><tbody>{filtered.map((record) => <tr key={record.id}><td>{record.document_type === "INVOICE" ? "Invoice" : record.document_type === "CR_NOTE" ? "CR Note" : "DR Note"}</td><td>{record.invoice_no}</td><td><strong>{record.vat_customer_name}</strong></td><td className={!record.customer_trn ? "vat-missing" : ""}>{record.customer_trn || "Missing"}</td><td className={!record.customer_address ? "vat-missing" : ""}>{record.customer_address || "Missing"}</td><td>{dateLabel(record.sales_date)}</td><td>{money(Math.abs(record.sales_item_total))}</td><td>{money(Math.abs(record.vat_amount || 0))}</td><td>{money(Math.abs(record.table_tax_amount || 0))}</td><td>{record.tax_classification === "VAT_TABLE" ? "VAT + Table" : record.tax_classification}</td><td>{money(record.wht)}</td><td><strong>{money(Math.abs(record.total_sales))}</strong></td></tr>)}</tbody></table></div>{!filtered.length && <div className="vat-empty">No records in the selected period.</div>}</section>
