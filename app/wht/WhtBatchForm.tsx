...
export default function WhtBatchForm({ customers, invoices }: { customers: string[]; invoices: Invoice[] }) {
  ...
  return <section className="wht-form-card wht-batch-card">
    ...
    {customer && <><div className="cheque-allocation-summary"><span>Collected WHT <strong>EGP {money(Number(collectedAmount))}</strong></span><span>Allocated <strong>EGP {money(allocatedTotal)}</strong></span><span className={allocationValid ? "balanced" : "unbalanced"}>Customer Unallocated WHT <strong>EGP {money(Math.max(0, unallocated))}</strong></span></div><div className="cheque-allocation-tools"><button type="button" disabled={!selectedInvoices.length || !Number(collectedAmount)} onClick={autoAllocate}>Auto Allocate WHT</button><span>Invoices are capped at expected WHT. Any excess remains as the customer's unallocated WHT balance.</span></div><div className="wht-batch-list">{customerInvoices.map((invoice) => { const invoiceNo = String(invoice.invoice_no); const wht = invoiceWht(invoice); return <div className="wht-batch-row" key={invoiceNo}><input type="checkbox" checked={Boolean(selected[invoiceNo])} onChange={() => toggle(invoiceNo)} /><span><strong>Invoice {invoiceNo}</strong><small>{invoice.sales_date} · Subtotal {money(invoice.sales_item_total)} EGP · Expected WHT {money(wht)} EGP</small></span><input type="number" min="0" max={wht} step="0.01" disabled={!selected[invoiceNo]} placeholder="Allocation" value={allocations[invoiceNo] ?? ""} onChange={(event) => setAllocations((current) => ({ ...current, [invoiceNo]: event.target.value }))} /></div>; })}{!customerInvoices.length && <p className="empty-state">No invoices found for this customer.</p>}</div></>}
