...
function linkedCustomerNames(customer: string) {
  return linkedCustomerGroups.find((group) => group.includes(customer)) ?? [customer];
}
...
export default function WhtBatchForm({ customers, invoices }: { customers: string[]; invoices: Invoice[] }) {
  ...
  const customerInvoices = useMemo(() => {
    const names = new Set(linkedCustomerNames(customer));
    return invoices
      .filter((invoice) => names.has(invoice.customer_name))
      .sort((a, b) => a.sales_date.localeCompare(b.sales_date) || String(a.invoice_no).localeCompare(String(b.invoice_no), undefined, { numeric: true }));
  }, [customer, invoices]);
  ...
  return <section className="wht-form-card wht-batch-card">
    ...
    {customer && <><div className="cheque-allocation-summary"><span>Collected WHT <strong>EGP {money(Number(collectedAmount))}</strong></span><span>Allocated <strong>EGP {money(allocatedTotal)}</strong></span><span className={allocationValid ? "balanced" : "unbalanced"}>Customer Unallocated WHT <strong>EGP {money(Math.max(0, unallocated))}</strong></span></div>{linkedCustomerNames(customer).length > 1 && <p className="wht-linked-customers">Showing invoices for: <strong>{linkedCustomerNames(customer).join(" + ")}</strong></p>}<div className="cheque-allocation-tools"><button type="button" disabled={!selectedInvoices.length || !Number(collectedAmount)} onClick={autoAllocate}>Auto Allocate WHT</button><span>Invoices and debit notes are capped at expected WHT. Any excess remains as the customer's unallocated WHT balance.</span></div><div className="wht-batch-list">{customerInvoices.map((invoice) => { const documentId = String(invoice.id); const wht = invoiceWht(invoice); const documentLabel = invoice.document_type === "DR_NOTE" ? "DR Note" : "Invoice"; return <div className="wht-batch-row" key={documentId}><input type="checkbox" checked={Boolean(selected[documentId])} onChange={() => toggle(documentId)} /><span><strong>{documentLabel} {invoice.invoice_no}</strong><small>{invoice.customer_name} · {invoice.sales_date} · Subtotal {money(invoice.sales_item_total)} EGP · Expected WHT {money(wht)} EGP</small></span><input type="number" min="0" max={wht} step="0.01" disabled={!selected[documentId]} placeholder="Allocation" value={allocations[documentId] ?? ""} onChange={(event) => setAllocations((current) => ({ ...current, [documentId]: event.target.value }))} /></div>; })}{!customerInvoices.length && <p className="empty-state">No invoices or debit notes found for this customer.</p>}</div></>}
