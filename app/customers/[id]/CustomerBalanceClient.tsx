...
type Invoice = { id: string | number; invoice_no: string; original_invoice_no?: string | null; sales_date: string; due_date: string | null; document_type: "INVOICE" | "CR_NOTE" | "DR_NOTE"; sent_to_hospital: boolean; balance_discount: number; balance_discount_reason: string | null; total_sales: number; expected_wht: number; collected_wht: number; customer_payments: number; cash_fraction: number; remaining_wht: number; remaining_money: number };
...
export default function CustomerBalanceClient({ customer, invoices, customerCreditBalance, unallocatedChequeBalance, unallocatedCheques, transferFeeAdjustments, canEditCustomers }: { customer: Customer; invoices: Invoice[]; customerCreditBalance: number; unallocatedChequeBalance: number; unallocatedCheques: UnallocatedCheque[]; transferFeeAdjustments: number; canEditCustomers: boolean }) {
  const [lang, setLang] = useState<"en" | "ar">("en");
  const [search, setSearch] = useState("");
  const [filter, setFilter] = useState<BalanceFilter>("OPEN");
  const [documentFilter, setDocumentFilter] = useState<DocumentFilter>("ALL");
  const [sentFilter, setSentFilter] = useState<SentFilter>("ALL");
  const [sentStatuses, setSentStatuses] = useState<Record<string, boolean>>(() => Object.fromEntries(invoices.map((invoice) => [String(invoice.id), Boolean(invoice.sent_to_hospital)])));
  const [sentSavingId, setSentSavingId] = useState<string | null>(null);
  ...
  const totals = useMemo(() => invoices.reduce((sum, item) => ({ total: sum.total + Number(item.total_sales), payments: sum.payments + item.customer_payments, fractions: sum.fractions + item.cash_fraction, discounts: sum.discounts + item.balance_discount, expectedWht: sum.expectedWht + item.expected_wht, collectedWht: sum.collectedWht + item.collected_wht, remainingMoney: sum.remainingMoney + item.remaining_money, remainingWht: sum.remainingWht + item.remaining_wht }), { total: 0, payments: 0, fractions: 0, discounts: 0, expectedWht: 0, collectedWht: 0, remainingMoney: 0, remainingWht: 0 }), [invoices]);
  ...
  const filtered = useMemo(() => invoices.filter((invoice) => {
    ...
    const isSent = Boolean(sentStatuses[String(invoice.id)]);
    const matchesSent = sentFilter === "ALL" || (invoice.document_type === "INVOICE" && (sentFilter === "SENT" ? isSent : !isSent));
    return matchesDocument && matchesFilter && matchesSent && (!search.trim() || String(invoice.invoice_no).toLowerCase().includes(search.trim().toLowerCase()));
  }), [invoices, documentFilter, filter, search, sentFilter, sentStatuses, today]);

  async function updateSentStatus(invoice: Invoice, checked: boolean) {
    const id = String(invoice.id);
    const previous = Boolean(sentStatuses[id]);
    setSentStatuses((current) => ({ ...current, [id]: checked }));
    setSentSavingId(id);
    try {
      const response = await fetch(`/api/sales/${id}/sent-to-hospital`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ sentToHospital: checked }),
      });
      const result = await response.json().catch(() => ({}));
      if (!response.ok) {
        setSentStatuses((current) => ({ ...current, [id]: previous }));
        alert(result.error || "Unable to update sent status.");
      }
    } finally {
      setSentSavingId(null);
    }
  }
  function selectDiscountInvoice(id: string) {
    const invoice = invoices.find((item) => String(item.id) === id);
    setDiscountInvoiceId(id);
    setDiscountAmount(invoice ? Number(invoice.balance_discount || 0).toFixed(2) : "");
    setDiscountReason(invoice?.balance_discount_reason || "");
    setDiscountMessage("");
  }

  async function saveDiscount() {
    const invoice = invoices.find((item) => String(item.id) === discountInvoiceId);
    if (!invoice) return setDiscountMessage("Select an invoice.");
    const amount = Number(discountAmount);
    if (!Number.isFinite(amount) || amount < 0 || amount > invoice.remaining_money + invoice.balance_discount + 0.005) {
      return setDiscountMessage(`Enter an amount up to EGP ${money(invoice.remaining_money + invoice.balance_discount)}.`);
    }
    if (amount > 0 && !discountReason.trim()) return setDiscountMessage("Enter a reason for the discount.");
    setDiscountSaving(true);
    setDiscountMessage("");
    try {
      const response = await fetch(`/api/sales/${discountInvoiceId}/balance-discount`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ amount, reason: discountReason.trim() }) });
      const result = await response.json().catch(() => ({}));
      if (!response.ok) return setDiscountMessage(result.error || "Unable to save discount.");
      window.location.reload();
    } catch {
      setDiscountMessage("Unable to save discount. Please try again.");
    } finally {
  ...
  async function downloadPdf() {
    ...
    const reportTotals = filtered.reduce((sum, item) => ({ total: sum.total + Number(item.total_sales), payments: sum.payments + item.customer_payments, fractions: sum.fractions + item.cash_fraction, discounts: sum.discounts + item.balance_discount, wht: sum.wht + item.expected_wht, collectedWht: sum.collectedWht + item.collected_wht, moneyDue: sum.moneyDue + item.remaining_money, whtDue: sum.whtDue + item.remaining_wht }), { total: 0, payments: 0, fractions: 0, discounts: 0, wht: 0, collectedWht: 0, moneyDue: 0, whtDue: 0 });
    ...
    const cards = [["INVOICE TOTAL", reportTotals.total], ["PAYMENTS", reportTotals.payments], ["DISCOUNTS", reportTotals.discounts], ["WHT DEDUCTED", reportTotals.wht], ["MONEY DUE", reportTotals.moneyDue], ["WHT DUE", reportTotals.whtDue]] as const;
  ...
  return <div className="dashboard-shell" dir={lang === "ar" ? "rtl" : "ltr"}>
    ...
    <main className="customer-balance-page">
      ...
      {canEditCustomers && <section className="customer-balance-table-card customer-balance-discount-panel">
        <div><h2>Invoice Balance Discount</h2><p>Internal receivables adjustment only. Original invoice sales and VAT remain unchanged.</p></div>
        <div className="customer-balance-discount-fields">
          <label><span>Invoice</span><select value={discountInvoiceId} onChange={(event) => selectDiscountInvoice(event.target.value)}><option value="">Select invoice</option>{invoices.filter((item) => item.document_type === "INVOICE" && (item.remaining_money > 0.005 || item.balance_discount > 0)).map((item) => <option key={String(item.id)} value={String(item.id)}>Invoice {item.invoice_no} · Remaining EGP {money(item.remaining_money)}</option>)}</select></label>
          <label><span>Discount (EGP)</span><input type="number" min="0" step="0.01" value={discountAmount} onChange={(event) => setDiscountAmount(event.target.value)} disabled={!discountInvoiceId} /></label>
          <label><span>Reason</span><input type="text" maxLength={500} value={discountReason} onChange={(event) => setDiscountReason(event.target.value)} disabled={!discountInvoiceId} placeholder="Why is this balance discounted?" /></label>
          <button type="button" className="primary" disabled={!discountInvoiceId || discountSaving} onClick={saveDiscount}>{discountSaving ? "Saving…" : "Save Discount"}</button>
        </div>
        {discountMessage && <p role="alert" className="collection-form-error">{discountMessage}</p>}
      </section>}
      <section className="customer-balance-table-card">
        <div className="customer-balance-toolbar"><div><h2>Invoice Balance Statement</h2><span>{filtered.length} of {invoices.length} documents</span></div><div className="customer-balance-actions"><div className="customer-balance-filters"><select aria-label="Document type" value={documentFilter} onChange={(event) => setDocumentFilter(event.target.value as DocumentFilter)}><option value="ALL">All Documents</option><option value="INVOICE">Invoices</option><option value="CR_NOTE">Credit Notes</option><option value="DR_NOTE">Debit Notes</option></select><select aria-label="Balance status" value={filter} onChange={(event) => setFilter(event.target.value as BalanceFilter)}><option value="ALL">All Balances</option><option value="OPEN">Open Money Balances</option><option value="OVERDUE">Overdue Money</option><option value="PAID">Paid Documents</option><option value="WHT_PENDING">Paid – WHT Pending</option></select><select aria-label="Sent to hospital status" value={sentFilter} onChange={(event) => setSentFilter(event.target.value as SentFilter)}><option value="ALL">All Sent Statuses</option><option value="SENT">Sent to Hospital</option><option value="NOT_SENT">Not Sent</option></select><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search document number" /></div><button className={`secondary customer-balance-wht-toggle ${showWht ? "is-active" : ""}`} type="button" onClick={() => setShowWht((value) => !value)} aria-pressed={showWht}>{showWht ? "Hide WHT" : "Show WHT"}</button><button className="primary customer-balance-pdf" type="button" onClick={downloadPdf}>Download PDF</button></div></div>
        <div className="table-scroll"><table className="customer-balance-table"><thead><tr><th>Invoice No.</th><th>Invoice Date</th><th>Due Date</th><th>Sent to Hospital</th><th className="number">Invoice Total</th><th className="number">Payments</th><th className="number">Cash Fraction</th><th className="number">Balance Discount</th>{showWht && <><th className="number">Expected WHT</th><th className="number">Collected WHT</th></>}<th className="number money-balance">Remaining Money</th>{showWht && <th className="number wht-balance">Remaining WHT</th>}<th>Status</th></tr></thead><tbody>{filtered.map((invoice) => { const moneyOpen = invoice.remaining_money > 0.005; const whtPending = !moneyOpen && invoice.remaining_wht > 0.005; const overdue = moneyOpen && Boolean(invoice.due_date && invoice.due_date < today); const invoiceId = String(invoice.id); return <tr key={invoiceId}><td><a className="invoice-number-link" href={`/sales/${invoice.id}`}>{invoice.invoice_no}</a></td><td>{dateLabel(invoice.sales_date)}</td><td className={overdue ? "date-overdue" : ""}>{dateLabel(invoice.due_date)}</td><td>{invoice.document_type === "INVOICE" ? <label className="sent-to-hospital-control"><input type="checkbox" checked={Boolean(sentStatuses[invoiceId])} disabled={sentSavingId === invoiceId} onChange={(event) => updateSentStatus(invoice, event.target.checked)} /><span>{sentStatuses[invoiceId] ? "Sent" : "Not Sent"}</span></label> : <span>—</span>}</td><td className="number">{money(invoice.total_sales)}</td><td className="number">{money(invoice.customer_payments)}</td><td className="number">{money(invoice.cash_fraction)}</td><td className="number" title={invoice.balance_discount_reason || undefined}>{invoice.balance_discount ? money(invoice.balance_discount) : "—"}</td>{showWht && <><td className="number">{money(invoice.expected_wht)}</td><td className="number">{money(invoice.collected_wht)}</td></>}<td className="number money-balance"><strong>{money(invoice.remaining_money)}</strong></td>{showWht && <td className="number wht-balance"><strong>{money(invoice.remaining_wht)}</strong></td>}<td><span className={`balance-status ${overdue ? "overdue" : moneyOpen ? "open" : whtPending ? "wht-pending" : "settled"}`}>{overdue ? "Overdue" : moneyOpen ? "Open" : whtPending ? "Paid · WHT Pending" : "Paid"}</span></td></tr>; })}{!filtered.length && <tr><td colSpan={showWht ? 13 : 10} className="collection-empty">No invoices match the selected filter.</td></tr>}</tbody><tfoot><tr><td colSpan={4}>Filtered Total</td><td className="number">{money(filtered.reduce((sum, item) => sum + Number(item.total_sales), 0))}</td><td className="number">{money(filtered.reduce((sum, item) => sum + item.customer_payments, 0))}</td><td className="number">{money(filtered.reduce((sum, item) => sum + item.cash_fraction, 0))}</td><td className="number">{money(filtered.reduce((sum, item) => sum + item.balance_discount, 0))}</td>{showWht && <><td className="number">{money(filtered.reduce((sum, item) => sum + item.expected_wht, 0))}</td><td className="number">{money(filtered.reduce((sum, item) => sum + item.collected_wht, 0))}</td></>}<td className="number money-balance">{money(filtered.reduce((sum, item) => sum + item.remaining_money, 0))}</td>{showWht && <td className="number wht-balance">{money(filtered.reduce((sum, item) => sum + item.remaining_wht, 0))}</td>}<td /></tr></tfoot></table></div>
      </section>
