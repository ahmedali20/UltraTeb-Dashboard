...
export default function CustomerBalanceClient({ customer, invoices, unallocatedChequeBalance, unallocatedCheques }: { customer: Customer; invoices: Invoice[]; unallocatedChequeBalance: number; unallocatedCheques: UnallocatedCheque[] }) {
  ...
  const [docType, setDocType] = useState<"ALL" | "INVOICE" | "CR_NOTE" | "DR_NOTE">("ALL");
  ...
  const years = useMemo(() => Array.from(new Set(invoices.map((invoice) => String(invoice.sales_date || "").slice(0, 4)).filter(Boolean))).sort((a, b) => Number(b) - Number(a)), [invoices]);
  const filtered = useMemo(() => invoices.filter((invoice) => {
    ...
    const matchesYear = year === "ALL" || String(invoice.sales_date || "").slice(0, 4) === year;
    const matchesDocType = docType === "ALL" || invoice.document_type === docType;
    return matchesFilter && matchesYear && matchesDocType && (!search.trim() || String(invoice.invoice_no).toLowerCase().includes(search.trim().toLowerCase()));
  }), [invoices, filter, year, docType, search, today]);
  ...
  async function downloadPdf() {
    ...
    const cards = (showWht ? [["INVOICE TOTAL", reportTotals.total], ["PAYMENTS", reportTotals.payments], ["FRACTIONS", reportTotals.fractions], ["WHT DEDUCTED", reportTotals.wht], ["MONEY DUE", reportTotals.moneyDue], ["WHT DUE", reportTotals.whtDue]] : [["INVOICE TOTAL", reportTotals.total], ["PAYMENTS", reportTotals.payments], ["FRACTIONS", reportTotals.fractions], ["MONEY DUE", reportTotals.moneyDue]]) as ReadonlyArray<readonly [string, number]>;
    cards.forEach(([label, value], index) => {
      const perRow = showWht ? 3 : 2; const emphasizedFrom = showWht ? 4 : 3;
      const x = 15 + (index % perRow) * (perRow === 3 ? 61 : 91.5); const y = 64 + Math.floor(index / perRow) * 16; const emphasized = index >= emphasizedFrom;
      doc.setFillColor(emphasized ? 245 : 249, emphasized ? 240 : 249, emphasized ? 252 : 251); doc.setDrawColor(emphasized ? 190 : 224, emphasized ? 168 : 224, emphasized ? 225 : 230); doc.roundedRect(x, y, perRow === 3 ? 57 : 87.5, 12.5, 1.5, 1.5, "FD");
    ...
    const tableHead = showWht
      ? ["Document", "Date / Due Date", "Document Total", "Payments / Fraction", "WHT Deducted / Collected", "Money Due", "WHT Due", "Status"]
      : ["Document", "Date / Due Date", "Document Total", "Payments / Fraction", "Money Due", "Status"];
    const tableBody = filtered.map((invoice) => {
      const moneyOpen = invoice.remaining_money > 0.005; const whtPending = !moneyOpen && invoice.remaining_wht > 0.005; const overdue = moneyOpen && Boolean(invoice.due_date && invoice.due_date < today);
      const prefix = invoice.document_type === "DR_NOTE" ? "DR" : invoice.document_type === "CR_NOTE" ? "CR" : "INV";
      const status = overdue ? "OVERDUE" : moneyOpen ? "OPEN" : whtPending ? "WHT PENDING" : "PAID";
      const row = [`${prefix} ${invoice.invoice_no}`, `${dateLabel(invoice.sales_date)}\n${dateLabel(invoice.due_date)}`, money(invoice.total_sales), `${money(invoice.customer_payments)}\n${money(invoice.cash_fraction)}`];
      if (showWht) row.push(`${money(invoice.expected_wht)}\n${money(invoice.collected_wht)}`);
      row.push(money(invoice.remaining_money));
      if (showWht) row.push(money(invoice.remaining_wht));
      row.push(status);
      return row;
    });
    const tableFoot = showWht
      ? [["TOTAL", "", money(reportTotals.total), `${money(reportTotals.payments)}\n${money(reportTotals.fractions)}`, `${money(reportTotals.wht)}\n${money(reportTotals.collectedWht)}`, money(reportTotals.moneyDue), money(reportTotals.whtDue), ""]]
      : [["TOTAL", "", money(reportTotals.total), `${money(reportTotals.payments)}\n${money(reportTotals.fractions)}`, money(reportTotals.moneyDue), ""]];
    const columnStyles: { [key: string]: Partial<import("jspdf-autotable").Styles> } = showWht
      ? { 0: { cellWidth: 17, fontStyle: "bold" }, 1: { cellWidth: 25 }, 2: { cellWidth: 26, halign: "right" }, 3: { cellWidth: 28, halign: "right" }, 4: { cellWidth: 30, halign: "right" }, 5: { cellWidth: 22, halign: "right", fontStyle: "bold" }, 6: { cellWidth: 20, halign: "right", fontStyle: "bold" }, 7: { cellWidth: 17, halign: "center", fontStyle: "bold", fontSize: 6.2 } }
      : { 0: { cellWidth: 20, fontStyle: "bold" }, 1: { cellWidth: 28 }, 2: { cellWidth: 32, halign: "right" }, 3: { cellWidth: 34, halign: "right" }, 4: { cellWidth: 26, halign: "right", fontStyle: "bold" }, 5: { cellWidth: 20, halign: "center", fontStyle: "bold", fontSize: 6.2 } };
    autoTable(doc, {
      startY: 100,
      head: [tableHead],
      body: tableBody,
  ...
  return <div className="dashboard-shell" dir={lang === "ar" ? "rtl" : "ltr"}>
    ...
    <main className="customer-balance-page">
      ...
      <section className="customer-balance-table-card">
        <div className="customer-balance-toolbar"><div><h2>Invoice Balance Statement</h2><span>{filtered.length} of {invoices.length} invoices</span></div><div className="customer-balance-actions"><div className="customer-balance-filters"><select value={year} onChange={(event) => setYear(event.target.value)}><option value="ALL">All Years</option>{years.map((item) => <option key={item} value={item}>{item}</option>)}</select><select value={docType} onChange={(event) => setDocType(event.target.value as typeof docType)}><option value="ALL">All Types</option><option value="INVOICE">Invoices</option><option value="CR_NOTE">CR Notes</option><option value="DR_NOTE">DR Notes</option></select><select value={filter} onChange={(event) => setFilter(event.target.value as BalanceFilter)}><option value="ALL">All Invoices</option><option value="OPEN">Open Money Balances</option><option value="OVERDUE">Overdue Money</option><option value="PAID">Paid Invoices</option><option value="WHT_PENDING">Paid – WHT Pending</option></select><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search invoice number" /></div><button className="secondary customer-balance-wht-toggle" type="button" onClick={() => setShowWht((value) => !value)}>{showWht ? "Hide WHT Details" : "Show WHT Details"}</button><button className="primary customer-balance-pdf" type="button" onClick={downloadPdf}>Download PDF</button></div></div>
