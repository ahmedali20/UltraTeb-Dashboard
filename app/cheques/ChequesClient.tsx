...
type Allocation = { id: number; invoice_id: string; invoice_no: string; allocated_amount: number };
  ...
  return <div className="dashboard-shell" dir={lang === "ar" ? "rtl" : "ltr"}>
    ...
    <main className="cheques-page">
      ...
      <section className="cheques-table-card">
        ...
        <div className="table-scroll"><table><thead><tr><th>Collection Date</th><th>Cheque Date</th><th>Customer</th><th>Bank Name</th><th>Cheque No.</th><th>Invoices & Allocations</th><th>Amount</th><th>Status</th><th>Status Date</th><th>Notes</th>{canEdit && <th>Update Status</th>}</tr></thead><tbody>
          {filtered.map((cheque) => { const values = draft(cheque); return <tr key={cheque.id}><td>{dateLabel(cheque.collection_date)}</td><td>{dateLabel(cheque.cheque_date)}</td><td>{cheque.customer_name}</td><td>{cheque.bank_name || "—"}</td><td><strong>{cheque.cheque_no}</strong></td><td><div className="cheque-linked-invoices">{cheque.allocations.map((allocation) => <a key={allocation.id} href={`/sales/${allocation.invoice_id}`}><span>Invoice {allocation.invoice_no}</span><strong>EGP {money(allocation.allocated_amount)}</strong></a>)}</div></td><td>EGP {money(cheque.amount)}</td><td><span className={`cheque-badge cheque-badge--${cheque.cheque_status.toLowerCase()}`}>{labels[cheque.cheque_status]}</span></td><td>{dateLabel(cheque.cheque_status_date)}</td><td>{cheque.notes || "—"}</td>{canEdit && <td><div className="cheque-update"><select value={values.status} onChange={(event) => change(cheque.id, "status", event.target.value)}>{workflow.map((status) => <option key={status} value={status}>{labels[status]}</option>)}</select><input type="date" value={values.statusDate} onChange={(event) => change(cheque.id, "statusDate", event.target.value)} /><button className="primary" disabled={savingId === cheque.id || values.status === cheque.cheque_status} onClick={() => save(cheque)}>{savingId === cheque.id ? "Saving…" : "Save"}</button></div></td>}</tr>; })}
