...
export default async function ChequeDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!cheque) notFound();
  const { data: allocations, error: allocationError } = await supabase.from("cheque_allocations").select("id, invoice_id, invoice_no, allocated_amount, cash_fraction, wht_deducted_amount").eq("cheque_id", chequeId).order("id");
