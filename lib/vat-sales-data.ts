...
export async function loadVatSalesData(from?: string, to?: string, allowPre2026 = true) {
  ...
  let whtQuery = supabase.from("wht_collections").select("invoice_no, document_type, wht_amount, invoice_date");
  ...
  (wht ?? []).forEach((item) => {
    const key = `${item.document_type ?? "INVOICE"}:${item.invoice_no}`;
    whtMap.set(key, (whtMap.get(key) ?? 0) + Number(item.wht_amount || 0));
  });
  return (sales ?? []).map((sale) => {
    ...
    return {
      ...
      wht: whtMap.get(`${sale.document_type}:${sale.invoice_no}`) ?? 0,
    };
  });
}
